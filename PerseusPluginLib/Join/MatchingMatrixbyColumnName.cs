...
namespace PerseusPluginLib.Join
{
    ...
    public class MatchingMatrixbyColumnName : IMatrixMultiProcessing
    {
        ...
        private static string[] SpecialSort3(IList<string> x, IList<string> y, IList<string> z, out Dictionary<string, int> xdic,
        out Dictionary<string, int> ydic, out Dictionary<string, int> zdic)
        {
            HashSet<string> hx = ArrayUtils.ToHashSet(x);
            HashSet<string> hy = ArrayUtils.ToHashSet(y);
        ...
        {
            IMatrixData resultinput = PerseusFactory.CreateMatrixData();
            if (nummatrix == 2)
            {
            ...
            else if (nummatrix == 3)
            {
                ...
                string[] expColNames = SpecialSort3(mdata1.ColumnNames, mdata2.ColumnNames, mdata3.ColumnNames, out Dictionary<string, int> dic1, out Dictionary<string, int> dic2, out Dictionary<string, int> dic3);
                double[,] ex = new double[nrows, expColNames.Length];
                for (int i = 0; i < ex.GetLength(0); i++)
                {
                    for (int j = 0; j < ex.GetLength(1); j++)
                ...
                string[] numColNames = SpecialSort3(mdata1.NumericColumnNames, mdata2.NumericColumnNames, mdata3.NumericColumnNames, out dic1, out dic2, out dic3);
                List<double[]> numCols = new List<double[]>();
                for (int i = 0; i < numColNames.Length; i++)
                {
                    numCols.Add(new double[nrows]);
                ...
                string[] stringColNames = SpecialSort3(mdata1.StringColumnNames, mdata2.StringColumnNames, mdata3.StringColumnNames, out dic1, out dic2, out dic3);
                List<string[]> stringCols = new List<string[]>();
                for (int i = 0; i < stringColNames.Length; i++)
                {
                    stringCols.Add(new string[nrows]);
                ...
                string[] catColNames = SpecialSort3(mdata1.CategoryColumnNames, mdata2.CategoryColumnNames, mdata3.CategoryColumnNames, out dic1, out dic2, out dic3);
                List<string[][]> catCols = new List<string[][]>();
                for (int i = 0; i < catColNames.Length; i++)
                {
                    catCols.Add(new string[nrows][]);
                ...
                string[] multiNumColNames = SpecialSort3(mdata1.MultiNumericColumnNames, mdata2.MultiNumericColumnNames, mdata3.MultiNumericColumnNames, out dic1,
                    out dic2, out dic3);
                List<double[][]> multiNumCols = new List<double[][]>();
                for (int i = 0; i < multiNumColNames.Length; i++)
                {
